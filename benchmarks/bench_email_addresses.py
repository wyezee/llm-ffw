"""Benchmark opt-in email inspection at the production payload envelope."""
...
def benchmark(
    *,
    ...
    workloads = (
        ...
        (
            "invalid_at_end",
            _sized_suffix(size, " user@example.1"),
            Action.ALLOW,
        ),
        (
            "email_at_end",
            _sized_suffix(size, " user@example.com"),
            Action.REDACT,
        ),
    )
    ...
    for round_number in range(rounds):
        ...
        for name, text, expected in ordered:
            ...
            if result.decision is not expected:
                raise RuntimeError("unexpected email benchmark decision")
    ...
    if measured.decision is not Action.REDACT:
        raise RuntimeError("email memory workload was not sanitized")
    ...
    pool = ProcessScannerPool(
        ...
        email_address_config=address_config,
    )
    with pool:
        if (
            pool.process(
            ...
            raise RuntimeError("process warm-up did not sanitize email")
        started = time.perf_counter()
    ...
    if any(item.decision is not Action.REDACT for item in results):
        raise RuntimeError("process benchmark did not sanitize email")
...
def main() -> None:
    ...
    parser.add_argument("--min-email-throughput-mib-s", type=float, default=2.5)
    ...
    if min(
        ...
        result["overlong_local_mib_per_second"],
        result["invalid_at_end_mib_per_second"],
    ) < args.min_throughput_mib_s:
        raise SystemExit("email throughput gate failed")
    if min(
        result["marker_dense_mib_per_second"],
        result["email_at_end_mib_per_second"],
    ) < args.min_email_throughput_mib_s:
        raise SystemExit("email redaction throughput gate failed")
    if result["address_peak_mib"] > args.max_peak_mib:
        raise SystemExit("email memory gate failed")
    if (
        result["process_requests_per_second"]
        < args.min_process_requests_per_second
    ):
        raise SystemExit("email process gate failed")
